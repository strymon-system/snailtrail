...
impl<K: Eq, V> PositionalMap<K, V> for Vec<Option<(K, V)>> {
    ...
    fn find(&self, k: &K) -> Option<(usize, &V)> {
        self.iter()
            .enumerate()
            .filter_map(|(i, cell)| {
                cell.as_ref().and_then(|&(ref cell_key, ref value)| {
                    if k == cell_key {
                        Some((i, value))
                    } else {
                        None
    ...
    fn find_mut(&mut self, k: &K) -> Option<(usize, &mut V)> {
        self.iter_mut()
            ...
            .filter_map(|(i, cell)| {
                cell.as_mut()
                    .and_then(|&mut (ref cell_key, ref mut value)| {
                        if k == cell_key {
                            Some((i, value))
                        } else {
                            None
...
fn main() {
    ...
    let skip_ts: Option<u64> = matches
        .value_of("skip-ts")
    ...
    let (records, workers) = {
        let mut records = pag_construction::input::read_sorted_trace_from_file_and_cut_messages(
            log_path,
            message_delay,
        );
    ...
    let ts_base = skip_ts.unwrap_or(
        worker_local_records()
            .next()
            .expect("expected one event")
            .timestamp,
    );
    eprintln!(
        "timestamp base  at {} ({} ms)",
        ts_base,
        ts_base / 1_000_000
    );
    ...
    for record in worker_local_records() {
        ...
        match record.event_type {
            EventType::Start => {
                let (pos, removed) = timeline.open_activities.positional_insert(
                    record.correlator_id.expect("correlator_ids expected"),
                    OpenActivity {
                        record: record.clone(),
                    },
                );
            ...
            EventType::End => {
                ...
                if record.timestamp >= ts_base {
                    groups.push(render_activity(
                        offset(worker_pos, pos),
                        width(),
        ...
        .filter(|r| !r.activity_type.is_worker_local())
    {
        match record.event_type {
            ...
            EventType::Received => {
                ...
                if record.timestamp >= ts_base {
                    groups.push(render_message(
                        worker_offset(start_worker_pos),
    ...
    eprintln!(
        "last visible timestamp {}, {} visible events",
        last_ts,
        groups.len()
    );
    ...
    let document = document.add((0..num_workers).fold(
    ...
    let document = document.add(
        (0..((last_ts - ts_base) / tick_distance_ts()))
