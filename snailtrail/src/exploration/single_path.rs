...
use exploration::rand::{thread_rng, Rng};
...
impl<G: Scope, N: ExtendedData + Partitioning, D1: SrcDst<N> + Data + Eq + Hash + Debug + Send>
    SinglePath<G, N, D1> for Stream<G, D1>
{
    fn single_path(&self, edge: &Stream<G, D1>) -> Stream<G, D1>
    where
        G::Timestamp: Hash + Copy,
    {
        ...
        let path = self.scope().scoped(|inner| {
            ...
            let output = graph_stream.enter(inner).traverse_from(
                &seed,
                |e| e.src(),
                |e| e.dst(),
                |t| &t.outer,
...
impl<
    ...
    > TraverseFrom<G, D1, K> for Stream<G, D1>
{
    fn traverse_from<LG, LJ, TO, TS>(
        &self,
        seed: &Stream<G, D1>,
